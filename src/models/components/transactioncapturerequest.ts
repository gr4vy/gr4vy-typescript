/*
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

import * as z from "zod";

/**
 * A request to capture a transaction.
 */
export type TransactionCaptureRequest = {
    /**
     * The monetary amount to capture an authorization for, in the smallest
     *
     * @remarks
     * currency unit for the given currency, for example `1299` cents to create
     * an authorization for `$12.99`.
     *
     * When omitted blank, this will capture the entire amount.
     *
     * Capturing an amount that is greater than the authorized amount is not
     * supported.
     */
    amount?: number | undefined;
};

/** @internal */
export namespace TransactionCaptureRequest$ {
    export type Inbound = {
        amount?: number | undefined;
    };

    export const inboundSchema: z.ZodType<TransactionCaptureRequest, z.ZodTypeDef, Inbound> = z
        .object({
            amount: z.number().int().optional(),
        })
        .transform((v) => {
            return {
                ...(v.amount === undefined ? null : { amount: v.amount }),
            };
        });

    export type Outbound = {
        amount?: number | undefined;
    };

    export const outboundSchema: z.ZodType<Outbound, z.ZodTypeDef, TransactionCaptureRequest> = z
        .object({
            amount: z.number().int().optional(),
        })
        .transform((v) => {
            return {
                ...(v.amount === undefined ? null : { amount: v.amount }),
            };
        });
}
