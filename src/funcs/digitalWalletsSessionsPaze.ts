 ...
 */
export function digitalWalletsSessionsPaze(
  client: Gr4vyCore,
...
> {
  const input: operations.CreatePazeDigitalWalletSessionRequest = {
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.CreatePazeDigitalWalletSessionRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON("body", payload.PazeSessionRequest, {
    explode: true,
  });

  const path = pathToFunc("/digital-wallets/paze/session")();
