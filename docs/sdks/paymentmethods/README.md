...
<!-- UsageSnippet language="typescript" operationID="update_payment_method" method="put" path="/payment-methods/{payment_method_id}" -->
```typescript
import { Gr4vy, withToken } from "@gr4vy/sdk";
import fs from "fs";
...
```typescript
import { Gr4vyCore } from "@gr4vy/sdk/core.js";
import { paymentMethodsUpdate } from "@gr4vy/sdk/funcs/paymentMethodsUpdate.js";
...
async function run() {
  const res = await paymentMethodsUpdate(gr4vy, {}, "ef9496d8-53a5-4aad-8ca2-00eb68334389");
  if (res.ok) {
    const { value: result } = res;
    console.log(result);
  } else {
